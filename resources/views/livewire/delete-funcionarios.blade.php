                ...
                <section>
                    ...
                    <form method="delete" class="mt-6 space-y-6" wire:submit.prevent="delete">
                        <div>
                            ...
                            <select name="funcionario" required wire:model="funcionario" class="mt-1 block w-full">
                                <option value="" selected disabled>Selecione</option>
                                @foreach ($funcionarios as $funcionario)
                                    <option value="{{$funcionario->id }}" >{{ $funcionario->id }} - {{ $funcionario->documento }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="flex items-center gap-4">
                            <button type="submit" class="bg-red-500 hover:bg-red-400 text-white font-bold py-2 px-4 border-b-4 border-red-700 hover:border-red-500 rounded">Excluir Funcionário</button>
